  ...
  >
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
        <!-- Brand section -->
        <div class="space-y-6 transition-all duration-700 {isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}">
          <h2 class="text-3xl font-bold text-shadow-neon-purple">SK8 Consign</h2>
          <p class="text-gray-400">Platform jual beli barang gaming terpercaya dengan pelayanan terbaik.</p>
          ...
          <div class="flex space-x-4">
            {#each socialLinks as social, index}
              <a 
                href={social.url}
                class="group transition-all duration-700 {isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}"
                style="transition-delay: {index * 100}ms"
                aria-label={social.name}
              >
                <div class="w-10 h-10 rounded-lg bg-black/50 border border-purple-500/30 
                           group-hover:border-purple-500/60 transition-all duration-300
                           flex items-center justify-center backdrop-blur-sm
                           group-hover:shadow-[0_0_15px_rgba(147,51,234,0.3)]">
                  <svg class="w-5 h-5 fill-current text-purple-400 group-hover:text-purple-300 transition-colors" viewBox="0 0 24 24">
                    <path d={social.icon} />
                  </svg>
                </div>
              </a>
        ...
        <!-- Quick Links -->
        <div class="space-y-6 transition-all duration-700 {isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}" style="transition-delay: 200ms">
        ...
        <!-- Contact Info -->
        <div class="space-y-6 transition-all duration-700 {isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}" style="transition-delay: 400ms">
          <h3 class="text-xl font-bold text-white">Hubungi Kami</h3>
          <ul class="space-y-3">
            {#each contactInfo as info}
              <li class="flex items-start space-x-3">
                <span class="text-purple-400 font-semibold">{info.label}:</span>
        ...
        <!-- Newsletter -->
        <div class="space-y-6 transition-all duration-700 {isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}" style="transition-delay: 600ms">
          ...
          <form class="space-y-3">
            <input 
              ...
              class="w-full px-4 py-2 rounded-lg bg-black/50 border border-purple-500/30 
                     text-white placeholder-gray-500 focus:outline-none focus:border-purple-500/60
                     transition-colors duration-300"
            />
            <button 
              type="submit"
              class="w-full px-4 py-2 rounded-lg bg-purple-600 hover:bg-purple-700 
      ...
      <!-- Bottom bar -->
      <div class="relative mt-16 pt-8 border-t border-purple-500/20">
        <div class="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
          <p class="text-gray-500 text-sm">
            © {new Date().getFullYear()} SK8 Consign. All rights reserved.
          </p>
          <div class="flex space-x-6">
            <a href="#" class="text-gray-500 hover:text-purple-400 text-sm transition-colors">Terms</a>
            <a href="#" class="text-gray-500 hover:text-purple-400 text-sm transition-colors">Privacy</a>
