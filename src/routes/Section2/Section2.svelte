...
>
	<section
		...
		class="rounded-t-[40px] bg-black px-4 py-24 shadow-2xl sm:px-6 lg:px-8"
	>
		<div class="mx-auto max-w-7xl">
			<h2
				class="mb-16 text-center text-4xl font-bold text-gray-100 transition-all duration-700 {isVisible
			...
			<div class="grid grid-cols-1 gap-8 md:grid-cols-3">
				{#each benefitCards as card, index}
					<div
						class="relative transform overflow-hidden rounded-lg shadow-lg transition-all duration-700 {isVisible
							? 'translate-y-0 opacity-100'
					...
					>
						...
						<div class="relative h-72 overflow-hidden">
							<div
								class="absolute inset-0 h-full w-full bg-cover bg-center transition-transform duration-500 hover:scale-110"
							...
							<div class="absolute inset-0 flex flex-col justify-end p-6 text-white">
								...
								<h4 class="mb-3 text-3xl font-bold">{card.subtitle}</h4>
								<p class="text-opacity-90 text-white">{card.description}</p>
							</div>
						</div>
					</div>
				{/each}
